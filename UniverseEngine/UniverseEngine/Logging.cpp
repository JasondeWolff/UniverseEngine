...
namespace UniverseEngine {
	...
	void Logging::Print(const char* level, const char* file, int line, const char* fmt, va_list args, bool showFile, bool crash)
	{
		char t[16000];
...

		{
			...
			if (showFile)
				fprintf(stdout, "[%s: %d]", f.substr(f.find_last_of("/\\") + 1).c_str(), line);
